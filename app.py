...
# DeepSeek API Key depuis l'environnement
DEESEEK_API_KEY = os.environ.get('DEESEEK_API_KEY')
DEESEEK_API_URL = "https://api.deepseek.com/v1/query"

def query_deepseek(prompt: str):
    if not DEESEEK_API_KEY:
        return "Erreur: DeepSeek API Key non définie."
    headers = {
    ...
    try:
        response = requests.post(DEESEEK_API_URL, headers=headers, json=payload, timeout=15)
...
def handle_reti(chat_id, value):
    ...
    if 200 <= state['step'] <= 202:
        ...
        if state['step'] < 202:
            ...
            calculations_history.append({'type': 'reticulocytes','result': taux,'timestamp': datetime.now().isoformat()})
...
def handle_plaquettes(chat_id, value):
    ...
    if state['step'] == 303:
        ...
        calculations_history.append({'type': 'platelets','result': result,'timestamp': datetime.now().isoformat()})
    ...
    try:
        if state['step'] == 400:
            ...
            if text.lower() in ['annuler', 'cancel', 'إلغاء']:
                user_states[chat_id] = {'step': 0}
                send_message(chat_id, "✅ Calcul annulé.", get_main_keyboard())
            else:
                quantite = float(text)
                ...
                send_message(chat_id, message, get_main_keyboard())
                calculations_history.append({'type': 'dilution','result': f"{numer}/{denom}",'timestamp': datetime.now().isoformat()})
                user_states[chat_id] = {'step': 0}
